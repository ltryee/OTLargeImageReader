...
+ (NSImage *)thumbImageFromLargeFile:(NSString *)filePath withConfirmedMaxPixelSize:(CGFloat)maxPixelSize
{
    // Create the image source (from path)
    CGImageSourceRef src = CGImageSourceCreateWithURL((__bridge CFURLRef) [NSURL fileURLWithPath:filePath], NULL);
    
    // To create image source from NSImage, use this
    // NSData* pngData =  NSImagePNGRepresentation(image);
    ...
    NSImage *image = [[NSImage alloc] initWithCGImage:thumbnail size:CGSizeZero];
    CFRelease(thumbnail);
    return image;
}
...
+ (NSImage *)thumbImageFromLargeFile:(NSString *)filePath withMaxPixelSize:(CGFloat)maxPixelSize imageSize:(CGSize)imageSize
{
    ...
    NSImage *image = [self thumbImageFromLargeFile:filePath withConfirmedMaxPixelSize:maxPixelSize];
    return image;
}
...
+ (void)thumbImageFromLargeFile:(NSString *)filePath withMaxPixelSize:(CGFloat)maxPixelSize imageSize:(CGSize)imageSize callback:(void(^)(NSImage *))callback
{
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        NSImage *image = [self thumbImageFromLargeFile:filePath withMaxPixelSize:maxPixelSize imageSize:imageSize];
        callback(image);
    });
}
...
{
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        NSImage *image = [self thumbImageFromLargeFile:filePath withMinPixelSize:minPixelSize imageSize:imageSize];
        callback(image);
    });
}
